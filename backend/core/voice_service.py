            ...
            async def _receiver() -> None:
                while True:
                    ...
                    if event == "task-failed":
                        ...
                        raise RuntimeError(f"Streaming TTS task failed: {message}")
    ...
    try:
        if not transcript:
            ...
            await _queue_voice_ws_event(
                ...
                transcript=_resolved_transcript(), reply_text=reply_text, exit_conversation=False,
            )
        ...
        await _queue_voice_ws_event(
            ...
            exit_conversation=_should_exit_ai_chat(_resolved_transcript()),
        )
        if metrics is not None:
            metrics.done_at = time.perf_counter()
            logger.info(
        ...
        await pending.event_queue.put(
            {
                ...
                "exit_conversation": _should_exit_ai_chat(pending.transcript),
            }
        )
    except Exception as exc:
...
async def create_voice_turn(
    ...
    *,
    ...
    if not transcript:
        ...
        exit_conversation = _should_exit_ai_chat(transcript)
